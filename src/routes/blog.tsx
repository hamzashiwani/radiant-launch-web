...
const tagsPool = ["Identity", "Technology", "Culture", "Process", "Field Notes", "Interview", "Toolkit", "Essay", "Type", "Motion", "Material", "Sound"];

const seed: Omit<Post, "id" | "image" | "accent" | "ts">[] = [
...
const posts: Post[] = seed.map((p, i) => ({
  ...p,
  id: `b${i + 1}`,
  image: images[i % images.length],
  accent: accents[i % accents.length],
  ts: seed.length - i, // newer first
}));

const allTags = Array.from(new Set(posts.map((p) => p.tag)));

const accentChip: Record<Post["accent"], string> = {
